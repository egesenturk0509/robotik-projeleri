...
export default function SignupPage() {
  ...
  const handleEmailSignup = async (email: string, password: string, displayName: string) => {
    ...
    try {
      ...
      await sendEmailVerification(userCredential.user); // Send verification email on signup
      setError('');
  ...
  const handleSocialSignup = async (provider: any, providerName: string) => {
    ...
    try {
      ...
      if (!isNewUser && result.user.email) { // If not a new user, guide them to login
        setError(
          <span>
      ...
      if (err.code) {
        switch (err.code) {
          ...
          case 'auth/account-exists-with-different-credential':
            errorMessage = `Bu e-posta adresi farklı bir sağlayıcı ile zaten kayıtlı. Lütfen mevcut hesabınızla giriş yapın.`;
            break;
          default:
            errorMessage = `${providerName} ile kayıt yapılırken bilinmeyen bir hata oluştu: ${err.message}`;
        }
      }
